...
User Action: "Create Contact"
    ↓
Home → Navigation → /contact-form/:offenderNumber
    ↓
ContactForm.ngOnInit()
    ↓
ContactData.getCaseloadOffenderById(offenderNumber)
    ↓
...
IndexedDB Insert → Contact Saved
    ↓
Navigation → /commentary-form/:offenderNumber/:contactId
```
...
User Action: "View Offender"
    ↓
MyCaseload → OffenderCard.click → /offender-detail/:offenderNumber
    ↓
OffenderDetail.ngOnInit()
    ↓
Parallel Data Loading:
├── ContactData.getCaseloadOffenderById(offenderNumber)
└── ContactData.getAllContactsByOffenderNumberDesc(offenderNumber)
    ↓
...
/contact-form/:offenderNumber/:contactId

// Components extract route parameters
this.route.snapshot.params['offenderNumber']
```
